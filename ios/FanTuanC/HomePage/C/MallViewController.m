...
- (void)rButtonAction:(UIButton *)btn
{
    ...
    searchVC.nameStr = _dic[@"title"];
    searchVC.mall_id = _dic[@"centerId"];
...
- (void)createTitleView
{
    ...
    [_dropMenu.otherButton setTitle:_dic[@"floorName"] forState:UIControlStateNormal];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (promotionArr.count == 0 && voucherGrouponArr.count == 0) {
        ...
        return 110 + promotionArr.count * 23 + 15;
    } else {
        if (![_listArr[indexPath.row][@"spider"] isEqualToString:@"0"]) {
...
- (void)getMallListData
{
    NSString *urlStr = [NetRequest MallList];
    NSDictionary *paramsDic = @{@"lng": [[NSUserDefaults standardUserDefaults] objectForKey:KLongitude], @"lat": [[NSUserDefaults standardUserDefaults] objectForKey:KLatitude], @"sort": _sort, @"cid": _cid, @"mall_id": _dic[@"centerId"], @"floor_id": _floor, @"pn": [NSString stringWithFormat:@"%ld", _pageNum]};
...
{
    __block MallViewController *mallVC = self;
    
    self.myTableView.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        
        mallVC.isUpLoading = YES;
        mallVC.pageNum++;
