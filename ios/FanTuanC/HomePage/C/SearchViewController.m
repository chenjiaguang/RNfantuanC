...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (![_mySearchBar.text isEqualToString:@""]) {
        [self getSearchData:_mySearchBar.text];
    }
}
...
- (void)viewDidLoad {
    ...
    _mall_hotsArr = [NSMutableArray array];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadSearchText:) name:reloadSearchText object:nil];
}
...
- (void)createSearchBar
{
    ...
    _mySearchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 7, titleView.frame.size.width - 65, 30)];
    _mySearchBar.showsCancelButton = NO;
    _mySearchBar.delegate = self;
    _mySearchBar.placeholder = @"可搜索商家、购物中心或分类";
...
#pragma mark - 热门购物中心
    CGFloat w2 = 5;
    CGFloat h2 = 60 + h + 40;
    for (int i = 0; i < _mall_hotsArr.count; i++) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
        button.tag = 200 + i;
        button.backgroundColor = [MyTool colorWithString:@"f5f5f5"];
        [button addTarget:self action:@selector(handleClick:) forControlEvents:UIControlEventTouchUpInside];
        ...
        NSDictionary *attributes = @{NSFontAttributeName:[UIFont systemFontOfSize:12]};
        CGFloat length = [[NSString stringWithFormat:@"  %@  ", _mall_hotsArr[i][@"name"]] boundingRectWithSize:CGSizeMake(kScreenW - 30, 2000) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size.width;
        //为button赋值
        [button setTitle: _mall_hotsArr[i][@"name"] forState:UIControlStateNormal];
        //设置button的frame
        button.frame = CGRectMake(10 + w2, h2, length + 10 , 26);
        //当button的位置超出屏幕边缘时换行 320 只是button所在父视图的宽度
        if(15 + w2 + length + 10 > kScreenW - 30){
            w2 = 15; //换行时将w置为0
            h2 = h2 + button.frame.size.height + 10;//距离父视图也变化
            button.frame = CGRectMake(w2, h2, length + 10, 26);//重设button的frame
        }
        w2 = button.frame.size.width + button.frame.origin.x;
        [_myScrollView addSubview:button];
    }
    
    if (_mall_hotsArr.count != 0) {
        UILabel *line2Label = [[UILabel alloc] initWithFrame:CGRectMake(15, 20 + h + 40, 150, 20)];
        line2Label.text = @"热门购物中心";
        line2Label.textColor = [MyTool colorWithString:@"333333"];
        line2Label.font = [UIFont boldSystemFontOfSize:18];
        [_myScrollView addSubview:line2Label];
        _myScrollView.contentSize = CGSizeMake(kScreenW, h2 + 40);
    }
    
    
#pragma mark - 历史记录
    NSMutableArray *array = [[[NSUserDefaults standardUserDefaults] objectForKey:KHomePageSearchHistory] mutableCopy];
    if (array.count != 0) {
        CGFloat w3 = 5;
        CGFloat h3 = 60 + h2 + 40;
        if (_mall_hotsArr.count == 0 && _merchant_hotsArr.count == 0) {
            h3 = h;
        }
...
//                text = [NSString stringWithFormat:@"%@...", text];
        ...
        UILabel *line3Label = [[UILabel alloc] initWithFrame:CGRectMake(15, 20 + h2 + 40, 150, 20)];
        if (_mall_hotsArr.count == 0 && _merchant_hotsArr.count == 0) {
...
{
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:KHomePageSearchHistory];
    [self createScrollView];
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    if (_isSearchList) {
        [[NSNotificationCenter defaultCenter] postNotificationName:reloadSearchListView object:searchBar.text];
        [[NSNotificationCenter defaultCenter] postNotificationName:reloadSearchText object:searchBar.text];
        [self dismissViewControllerAnimated:NO completion:nil];
    } else {
        SearchListViewController *searchListVC = [[SearchListViewController alloc] init];
...
- (void)getSearchData
{
    ...
    [[NetToolExtend shareInstance] postWithURL:urlStr params:paramsDic success:^(id JSON) {
        if ([[JSON objectForKeyNotNull:@"error"] integerValue] == 0) {
            ...
            _mall_hotsArr = [JSON[@"data"][@"mall_hots"] mutableCopy];
...
{
    NSString *urlStr = [NetRequest SearchSuggest];
    NSDictionary *paramsDic = @{@"search_word": searchText, @"lng": _longitude, @"lat": _latitude};
...
- (void)saveHistoryDataWithText:(NSString *)text
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:array forKey:KHomePageSearchHistory];
...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:reloadSearchText object:nil];
}
