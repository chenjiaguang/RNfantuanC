//
//  DynamicDetailsFourPictureCell.m
//  FanTuanC
//
...
@interface DynamicDetailsFourPictureCell ()
{
...
@end
@implementation DynamicDetailsFourPictureCell
...
-(void)setModel:(ListModel *)model
{
    ...
    [_contentLabel  setAttributedText:setString];
     ...
     [_iamgeBgView setupAutoMarginFlowItems:[_imageViews subarrayWithRange:NSMakeRange(0, _model.covers.count)] withPerRowItemsCount:2 itemWidth:(kScreenW- 3)/2 verticalMargin:3 verticalEdgeInset:0 horizontalEdgeInset:0];
    for (int i = 0; i < _model.covers.count; i ++) {
        UIImageView *imageView = _imageViews[i];
        imageView.hidden = NO;
        [imageView yy_setImageWithURL:[NSURL URLWithString:_model.covers.firstObject.url] options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation];
    }
