//
//  DynamicDetailsOnePictureCell.m
//  FanTuanC
//
...
@interface DynamicDetailsOnePictureCell ()
{
...
@end
@implementation DynamicDetailsOnePictureCell
...
-(void)setLike_list:(NSArray<ListModel *> *)like_list
{
    ...
    for (int i =0 ; i< _like_list.count; i ++) {
        ...
        [imageView yy_setImageWithURL:[NSURL URLWithString:_likeImageViews[i]] options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation];
        imageView.hidden = NO;
    }
}
-(void)setModel:(ListModel *)model
{
    ...
    [_pictureImageView yy_setImageWithURL:[NSURL URLWithString:_model.covers.firstObject.url] options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation];

    _pictureImageView.sd_layout
    .heightIs(kScreenW * [_model.covers.firstObject.height floatValue]/[_model.covers.firstObject.width floatValue]);
...
-(void)onClickImage:(id)sender
{
    ...
    KSPhotoItem *item = [KSPhotoItem itemWithSourceView:_pictureImageView imageUrl:[NSURL URLWithString:_model.covers.firstObject.url]];
...
-(void)createUI
{
    ...
    UIImageView *pictureImageView = [YYAnimatedImageView new];
    pictureImageView.backgroundColor = [MyTool ColorWithColorStr:@"EEEEEE"];
    pictureImageView.userInteractionEnabled = YES;
    UITapGestureRecognizer *picSingleTap =[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(onClickImage:)];
    [pictureImageView addGestureRecognizer:picSingleTap];
    ...
    [self.contentView sd_addSubviews:@[headImageView,nameLabel,focusbnt,timerLabel,contentLabel,pictureImageView,praisebnt,commentsbnt,addressbnt,browseNumLabel,likeIamgeBgView,praiseNumLabel]];
    ...
    .leftSpaceToView(self.contentView, 15)
    .topSpaceToView(pictureImageView, 12);
