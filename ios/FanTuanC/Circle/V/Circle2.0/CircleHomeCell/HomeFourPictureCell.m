...
@interface HomeFourPictureCell ()
{
    ...
    UIView *_iamgeBgView;        //九宫格背景图
    UIView *_lienView;           //分割线
    UIImageView *_adminIamgeView;//管理员图标

}
@end
@implementation HomeFourPictureCell
...
//    _contentLabel.text = _model.content;
    ...
    for (int i = 0; i < _model.covers.count; i ++) {
        ...
        [imageView yy_setImageWithURL:[NSURL URLWithString:_model.covers[i].compress] options:YYWebImageOptionProgressiveBlur | YYWebImageOptionSetImageWithFadeAnimation];
    }
    ...
    [_iamgeBgView setupAutoMarginFlowItems:[_imageViews subarrayWithRange:NSMakeRange(0, _model.covers.count)]
                      withPerRowItemsCount:2
                                 itemWidth:(kScreenW-30-6)/3
                            verticalMargin:3
                         verticalEdgeInset:0
...
-(void)autoLayoutView
{
    ...
    .leftSpaceToView(self.contentView, 15)
    .topSpaceToView(_model.location.length > 0?_addressbnt:_iamgeBgView, 15)
    .heightIs(15)
    .widthIs(89);
...
-(void)createUI
{
    ...
    UIImageView *picIamgeView = [[UIImageView alloc]initWithFrame:CGRectZero];
    ...
    _lienView = lienView;
    [self.contentView sd_addSubviews:@[headImageView,nameLabel,adminIamgeView,timerLabel,contentLabel,praisebnt,commentsbnt,sourceLabel,sourcebnt,addressbnt,iamgeBgView,lienView]];
    [self autoLayoutView];
