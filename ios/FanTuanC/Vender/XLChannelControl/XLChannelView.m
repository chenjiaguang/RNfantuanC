...
static CGFloat CellMarginY = 16.0f;
...
-(void)buildUI{
    ...
    flowLayout.itemSize = CGSizeMake(cellWidth,cellWidth / 1.8f);
    ...
    flowLayout.headerReferenceSize = CGSizeMake(self.bounds.size.width, 47);
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    item.title = indexPath.section == 0 ? _inUseTitles[indexPath.row][@"name"] : _unUseTitles[indexPath.row][@"name"];
//    item.isFixed = indexPath.section == 0 && indexPath.row == 0;
    item.backgroundColor = indexPath.section == 0 ? [MyTool colorWithString:@"f5f5f5"] : [UIColor whiteColor];
    item.layer.borderWidth = indexPath.section == 0 ? 0.f : 0.5f;
    item.deleteImageV.hidden = indexPath.section == 0 ? NO : YES;
    return  item;
}
